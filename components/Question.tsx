...
export default function Question({
  ...
  return (
    <motion.div
      ...
      className="max-w-2xl mx-auto p-4 md:p-6 md:pt-6 bg-gray-900/60 backdrop-blur-md rounded-xl border border-yellow-400/20"
    >
      <div className="mb-8">
        ...
        <motion.h2
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="mt-4 text-xl md:text-2xl font-bold text-yellow-200"
        >
      ...
      <div className="space-y-3">
        {questionData.options.map((option, index) => (
          <motion.button
            ...
            className={`w-full p-4 text-left border-2 rounded-lg transition-all transform hover:scale-[1.01] ${
              showExplanation
      ...
      {showExplanation && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="mt-6 p-4 rounded-lg bg-gray-800/80 border border-yellow-400/30 backdrop-blur-sm"
        >
          <p className="font-bold text-lg mb-2">
            {selectedAnswer === questionData.correctAnswer ? (
              ...
              <span className="text-red-400">
                {t.incorrect} {questionData.options[questionData.correctAnswer]}
              </span>
            )}
          </p>
          ...
          <motion.button
            ...
            className="mt-4 w-full md:w-auto px-6 py-3 bg-gradient-to-r from-yellow-400 to-yellow-500 text-gray-900 rounded-lg hover:shadow-yellow-400/20 transition-all shadow-lg font-medium"
          >
            {t.nextQuestion}
          </motion.button>
        </motion.div>
